...
private const val MIRROR_TYPE = "rushtify-playlists"
...
/** Public Downloads/Rushtify JSON mirror. Room remains the fast runtime
...
class PlaylistPublicMirror @Inject constructor(
    ...
    suspend fun importAndMerge(content: String): Result<Int> = mutex.withLock {
        val mirror = decode(content)
            ?: return@withLock Result.failure(IllegalArgumentException("Invalid Rushtify playlist JSON"))
        try {
            dao.upsertAll(mirror.playlists.map { it.toEntity() })
