...
class LocalStatsRepository @Inject constructor(
    ...
    suspend fun load(): LocalStats {
        ...
        val allTracks = (playlists.flatMap { it.tracks } + liked)
            .filter { it.name.isNotBlank() && it.artist.isNotBlank() && !com.rushtify.app.util.ArtistHelper.isPlayCountOrStat(it.artist) }
        ...
        val albumSeeds = allTracks.mapNotNull { t ->
            ...
            val cleanArtist = d.artist.trim().takeIf { !com.rushtify.app.util.ArtistHelper.isPlayCountOrStat(it) && it.isNotBlank() } ?: "Unknown artist"
