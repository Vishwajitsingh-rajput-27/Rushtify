...
import com.rushtify.app.data.repository.ScrobbleRepository
import com.rushtify.app.data.repository.ThemeRepository
import com.rushtify.app.service.ScrobbleDebugLog
import com.rushtify.app.widget.ActiveMediaSessionHolder
...
class MusicPlaybackService : MediaBrowserServiceCompat() {
    ...
    @Inject lateinit var artworkRepository: com.rushtify.app.data.artwork.ArtworkRepository
    ...
    override fun onCreate() {
        ...
        playbackWifiLock = wifiManager?.createWifiLock(
            ...
            "Rushtify:PlaybackWifiLock",
        )?.apply { setReferenceCounted(false) }

        createNotificationChannel()
        runCatching {
            val session = MediaSessionCompat(this, "RushtifyPlayer").apply {
                setFlags(
        ...
        scope.launch {
            artworkRepository.resolved.collect { map ->
                ...
                val key = com.rushtify.app.data.artwork.ArtworkNormalizer.cacheKey(current.title, current.artist)
    ...
    override fun onGetRoot(
        ...
        // (onConnectionFailed), which is why KWGT never listed Rushtify as
    ...
    private fun promoteForPlayback(): Boolean {
        ...
        val promoted = runCatching {
            ...
            val minimal = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
             ...
             .setContentTitle(musicPlayer.state.value.current?.title ?: "Rushtify")
    ...
    private fun cleanTrackMetadata(title: String, artist: String): CleanTrackMetadata {
        ...
        var cleanArtist = artist.trim()
            ...
            .trim()
        if (com.rushtify.app.util.ArtistHelper.isPlayCountOrStat(cleanArtist)) {
            cleanArtist = ""
        }
    ...
    private fun requestArtwork(track: PlayableTrack?) {
        ...
        val key = com.rushtify.app.data.artwork.ArtworkNormalizer.cacheKey(track.title, track.artist)
    ...
    private fun buildNotification(state: MusicPlayerState, art: Bitmap?): Notification {
        ...
        if (useSystemMediaStyle()) {
            ...
            return builder
                .setSmallIcon(R.drawable.ic_launcher_logo)
                .setContentTitle(state.current?.title ?: "Rushtify")
    ...
    private fun notificationRemoteViews(
        ...
        setTextViewText(R.id.notification_title, state.current?.title ?: "Rushtify")
        ...
        runCatching {
            ...
            nm.createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Music playback", NotificationManager.IMPORTANCE_DEFAULT).apply {
                    description = "Native Rushtify playback controls"
                    setShowBadge(false)
                },
            )
        }.onFailure { error ->
    ...
    private companion object {
        ...
        const val CHANNEL_ID = "rushtify_playback_v2"
        const val LEGACY_CHANNEL_ID = "rushtify_playback"
        const val NOTIFICATION_ID = 4102
        const val ACTION_PREVIOUS = "com.rushtify.app.playback.PREVIOUS"
        const val ACTION_TOGGLE = "com.rushtify.app.playback.TOGGLE"
        const val ACTION_NEXT = "com.rushtify.app.playback.NEXT"
        const val ACTION_SHUFFLE = "com.rushtify.app.playback.SHUFFLE"
        const val ACTION_REPEAT = "com.rushtify.app.playback.REPEAT"
        const val ACTION_STOP = "com.rushtify.app.playback.STOP"
    }
}
