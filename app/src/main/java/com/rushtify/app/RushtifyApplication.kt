...
@HiltAndroidApp
class RushtifyApplication : Application(), ImageLoaderFactory {
    ...
    @Inject lateinit var streamExtractor: dagger.Lazy<com.rushtify.app.data.music.YouTubeStreamExtractor>
    @Inject lateinit var innerTubeMusicApi: dagger.Lazy<com.rushtify.app.data.music.InnerTubeMusicApi>
    @Inject lateinit var ytMusicSyncManager: dagger.Lazy<com.rushtify.app.data.ytmusic.YtMusicSyncManager>
    @Inject lateinit var ytMusicHistorySyncManager: dagger.Lazy<com.rushtify.app.data.ytmusic.YtMusicHistorySyncManager>
    @Inject lateinit var likedSongsManager: dagger.Lazy<com.rushtify.app.data.playlist.LikedSongsManager>
    @Inject lateinit var trackDownloadManager: dagger.Lazy<com.rushtify.app.data.download.TrackDownloadManager>
    @Inject lateinit var appLocaleManager: dagger.Lazy<com.rushtify.app.util.AppLocaleManager>
    ...
    override fun attachBaseContext(base: Context) {
        ...
        super.attachBaseContext(com.rushtify.app.util.AppLocaleManager.wrap(base))
    ...
    override fun onCreate() {
        ...
        runCatching { com.rushtify.app.playback.PlaybackDiagnostics.install(this) }
        runCatching { com.rushtify.app.data.music.potoken.BotGuardTokenGenerator.initialize(this) }
        applicationScope.launch(Dispatchers.IO) {
            ...
            runCatching { likedSongsManager.get().start() }
                .onFailure { android.util.Log.e("RushtifyStartup", "Liked Songs startup disabled", it) }
        }
        ...
        applicationScope.launch {
            ...
            runCatching { ytMusicSyncManager.get().start() }
                .onFailure { android.util.Log.e("RushtifyStartup", "YT sync startup disabled", it) }
        }
        ...
        applicationScope.launch {
            ...
            runCatching { ytMusicHistorySyncManager.get().start() }
                .onFailure { android.util.Log.e("RushtifyStartup", "YT history sync startup disabled", it) }
        }
        ...
        applicationScope.launch(Dispatchers.IO) {
            ...
            runCatching { trackDownloadManager.get().syncDownloadsFromStorage() }
                .onFailure { android.util.Log.e("RushtifyStartup", "Download sync startup failed", it) }
        }
        // A widget is a separate RemoteViews surface, so it needs an explicit
        // refresh whenever Rushtify's live theme changes. The widget's palette
        ...
        applicationScope.launch(Dispatchers.IO) {
            ...
            try {
                themeRepository.get().uiState
                    ...
                    .collect {
                        WidgetUpdater.refreshTheme(this@RushtifyApplication)
                    }
            } catch (cancellation: CancellationException) {
                throw cancellation
            } catch (error: Throwable) {
                android.util.Log.e("RushtifyStartup", "Widget theme observer disabled", error)
            }
        }
    }
    ...
    override fun newImageLoader(): ImageLoader {
        ...
        val imageClient = try {
            ...
            android.util.Log.e("RushtifyStartup", "Shared artwork client unavailable; using isolated client", error)
            okhttp3.OkHttpClient.Builder().build()
        } catch (error: LinkageError) {
            android.util.Log.e("RushtifyStartup", "Shared artwork client unsupported; using isolated client", error)
            okhttp3.OkHttpClient.Builder().build()
        }
